...
test_partition() {
    ...
    if docker exec slurmctld sinfo -h | grep -q "normal"; then
        print_info "Default partition 'normal' exists"
        PARTITIONS_FOUND=$((PARTITIONS_FOUND + 1))
    else
        print_fail "Default partition 'normal' not found"
    fi

    if docker exec slurmctld sinfo -h | grep -q "rapida"; then
        print_info "Specialized partition 'rapida' exists"
        PARTITIONS_FOUND=$((PARTITIONS_FOUND + 1))
    else
        print_fail "Specialized partition 'rapida' not found"
    fi

    if [ "$PARTITIONS_FOUND" -eq 2 ]; then
        print_pass "All required partitions are configured"
    else
        return 1
    fi
}
...
test_rest_api_nodes() {
    ...
    # Simplified check: just look for the nodes array
    if echo "$RESPONSE" | grep -q '"nodes"'; then
        print_info "  ✓ Response contains node data"
...
test_rest_api_partitions() {
    ...
    # Simplified check: just look for the partitions array
    if echo "$RESPONSE" | grep -q '"partitions"'; then
        print_info "  ✓ Response contains partition data"
